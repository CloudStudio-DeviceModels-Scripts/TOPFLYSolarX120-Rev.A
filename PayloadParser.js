function parseUplink(device, payload) {
    ...
    var tempBLT = device.endpoints.byAddress(3); 
    ...
    // BLE Door Sensor (temperatura y estado)
if (decodedMessage != null && decodedMessage.messageType === "bluetoothData" && decodedMessage.bleMessageType === "door") {
    var sensors = decodedMessage.bleDataList;
    for (var i = 0; i < sensors.length; i++) {
        var sensor = sensors[i];

        env.log("Bluetooth Door Sensor → MAC:", sensor.mac, " | Temp:", sensor.temp, "°C | Door Status:", sensor.doorStatus);
                    
        // Temperatura
        if (sensor.temp !== undefined && sensor.temp !== -999) {
            var eTemp = device.endpoints.byType(endpointType.temperatureSensor);
            if (eTemp != null) {
                //eTemp.updateTemperatureSensorStatus(sensor.temp, decodedMessage.date);
                tempBLT.updateTemperatureSensorStatus(sensor.temp,decodedMessage.date);
            }
        }
        /*
        // Estado de la puerta (0 cerrado, 1 abierto)
        if (sensor.doorStatus !== undefined && sensor.doorStatus !== -999) {
            var eDoor = device.endpoints.byType(endpointType.binarySensor); // Asegúrate de tener este endpoint
            if (eDoor != null) {
                //eDoor.updateBinarySensorStatus(sensor.doorStatus, decodedMessage.date);
            }
